...
**重要: Pythonのバージョンをdbt用とSnowflake CLI用で使い分けます。**

- `dbt deps` は社内証明書の関係で **Python 3.11.x** で実行する必要があります
- Snowflake CLI (`snow`) クライアントは **Python 3.12.x** を使います

この2系統を1つの仮想環境に混在させると証明書検証やパッケージ互換性の問題が起きるため、**pyenvで両バージョンを用意し、uvで用途ごとに別々の仮想環境／ツール環境を作る**構成にします。

| ツール | 用途 | バージョン |
|---|---|---|
| pyenv | Pythonバージョン管理（複数バージョン共存） | 最新 |
| Python (dbt用) | dbt-core / dbt-snowflake / `dbt deps` 実行基盤 | **3.11.x固定**（社内証明書の関係） |
| Python (Snowflake CLI用) | `snow` コマンド実行基盤 | **3.12.x** |
| uv | Python仮想環境・パッケージ管理（バージョンを指定して環境を分離） | 最新 |
...
├── .gitignore
├── README.md
├── pyproject.toml           # dbt用(Python 3.11.x)の依存関係定義
├── uv.lock
├── .venv/                    # dbt用仮想環境（Python 3.11.x） ※ uv venvで作成、.gitignore対象
├── Makefile                 # よく使うコマンドをラップ（venv切り替えを吸収）
├── .dbt/
...
### 4-1. pyenvで2つのPythonバージョンを入れる

dbt用（3.11.x）とSnowflake CLI用（3.12.x）の両方をpyenvにインストールしておきます。1台のマシンに複数バージョンを共存させ、用途ごとにuvで使い分ける方針です。
...
# dbt用（社内証明書の関係で3.11系固定）
pyenv install 3.11.9

# Snowflake CLI用
pyenv install 3.12.7

pyenv versions   # 両方が一覧に出ることを確認
```

> `pyenv global` / `pyenv local` でどちらか一方を既定にする必要はありません。次の手順のように、uvが各仮想環境作成時に明示的にバージョンを指定するため、pyenv側は「インストールしておくだけ」でOKです。
...
### 4-3. dbt用仮想環境（Python 3.11.x）の作成

リポジトリ直下で、pyenvで入れた3.11.9を明示的に指定して仮想環境を作ります。

```bash
uv venv .venv --python ~/.pyenv/versions/3.11.9/bin/python
source .venv/bin/activate

uv pip install dbt-core==1.10.* dbt-snowflake==1.10.*
dbt --version
python --version   # Python 3.11.9 であることを確認
```

`pyproject.toml` にも固定しておくと再現性が上がります:
...
```bash
uv sync --python ~/.pyenv/versions/3.11.9/bin/python
```

`dbt deps` は必ずこの3.11.x環境（`.venv`を有効化した状態）で実行してください。社内証明書の検証がこのバージョンでのみ通る前提のため、3.12系の環境で実行すると証明書エラーになる可能性があります。
...
### 4-4. Snowflake CLI (`snow`) のインストール（Python 3.12.x・独立環境）

`snow` はdbtの仮想環境とは**完全に分離**し、pyenvで入れた3.12.7を使って `uv tool` でインストールします。`uv tool install` はリポジトリの`.venv`とは別に、uv管理下の独立した実行環境を作るため、dbt側のPythonバージョンやパッケージ構成に影響しません。

```bash
uv tool install snowflake-cli --python ~/.pyenv/versions/3.12.7/bin/python

snow --version
uv tool run --from snowflake-cli python -c "import sys; print(sys.version)"
# → 3.12.x であることを確認
```

> 旧パッケージ名 `snowflake-cli-labs` は廃止されています。必ず `snowflake-cli` を使ってください。
> `pipx install snowflake-cli --python ~/.pyenv/versions/3.12.7/bin/python` でも同様のことができます（uv未導入の環境向け）。

### 4-5. 2系統の使い分けまとめ

| コマンド | 使う環境 | 実行前の準備 |
|---|---|---|
| `dbt deps` / `dbt run` / `dbt test` / `dbt build` / `dbt debug` | dbt用venv（Python 3.11.x） | `source .venv/bin/activate` |
| `snow dbt deploy` / `snow dbt execute` / `snow connection ...` | Snowflake CLI用（Python 3.12.x, uv tool） | 特になし（`uv tool install`済みなら`snow`が直接PATHに乗る） |

`.venv`を有効化した状態のまま`snow`コマンドも打てますが、`snow`自体は`uv tool`側の独立したPythonで動くため、`.venv`をactivateしているかどうかは`snow`の動作に影響しません。逆に`dbt`コマンドは必ず`.venv`をactivateした状態（＝3.11.x）で実行するよう徹底してください。
...
# dbt用環境（Python 3.11.x）のセットアップ
uv venv .venv --python ~/.pyenv/versions/3.11.9/bin/python
source .venv/bin/activate
uv sync --python ~/.pyenv/versions/3.11.9/bin/python

cd dbt_project
dbt deps          # ← 3.11.x環境で実行（社内証明書対応）
dbt debug          # ~/.dbt/profiles.ymlの接続確認
cd ..

# Snowflake CLI（Python 3.12.x, 独立環境）のセットアップ ※初回のみ
uv tool install snowflake-cli --python ~/.pyenv/versions/3.12.7/bin/python
snow connection add
```
